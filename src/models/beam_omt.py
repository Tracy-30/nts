...
class Beam():
    ...
    def __init__(self, size, device=False):
        ...
        self.next_ys = [torch.full((size,), cfg['PAD_idx'], dtype=torch.long, device=device)]
        self.next_ys[0][0] = cfg['SOS_idx']
    ...
    def advance(self, word_prob):
        ...
        if self.next_ys[-1][0].item() == cfg['EOS_idx']:
            self._done = True
            self.all_scores.append(self.scores)
    ...
    def get_tentative_hypothesis(self):
        ...
        if len(self.next_ys) == 1:
            ...
            hyps = [[cfg['SOS_idx']] + h for h in hyps]
    ...
    def translate_batch(self, src_seq):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, src_seq, enc_output, inst_idx_to_position_map, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
            ...
            def predict_word(dec_seq, dec_pos, src_seq, enc_output, n_active_inst, n_bm, enc_batch_extend_vocab, extra_zeros, mask_src):
                ## masking
                mask_trg = dec_seq.data.eq(cfg['PAD_idx']).unsqueeze(1)
        ...
        with torch.no_grad():
            ...
            mask_src = enc_batch.data.eq(cfg['PAD_idx']).unsqueeze(1)
            ...
            #-- Decode
            for len_dec_seq in range(1, cfg['max_dec_step'] + 1):
...
def get_input_from_batch(batch):
    ...
    c_t_1 = torch.zeros((batch_size, 2 * cfg['hidden_dim']))
